...
Most of the following functional requirements are derived from the use cases, to support the functionality that the customer has requested in the project description. A few requirements have been added and/or modified based on the abuse cases, however, and these are marked with a star (*) and mentioned here. They are not fully implemented yet but they are planned for a final release.
...
We have some assets that we think are worth protecting e.g. our database so we apply our principles to the operations.

\begin{description}
\item[Least Privilege]  The application server and database runs each with their respective user that is only used for that process. However we needed to host the application on port 80 so we configured it by using another tool.
...
\item[Reluctance to Trust]
We store the unique password to the database on the server and we don't share the username and password in our VCS according to the principle of \textbf{Reluctance to Trust} since we don't trust the confidentiality of our VCS.
...
\textbf{Least Privilege} as possible to the file system. We have disabled remote login for the root user and every member of the team has a user that is in a superuser group.
...
\item[PostgreSQL 9.1.9] is our chosen database technology. Among other things, it provides fine-grained user access control.
...
\item[Uptime Robot] A SaaS that monitors websites for downtime and sends email if they are unresponsive.
\end{description}
...
Raceconditions are prevented (see the log \ref{sec:appendix:racecondition}).
