...
A hacker would potentially only need a bridgehead on a server in order to intrude a server. An application can only be as secure as its weakest link.

In order to establish a bridgehead an attacker would discover the technologies and frameworks that the server an application runs on uses. Over time software tends to degrade in security and an important discovery is the particular version of a technology. With this information it is possible to look in open exploit- and bug databases\cite{CVEdb} to search for entries that might impact the security of the application, and then try to execute attacks based on the bugs.
