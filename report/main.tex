...
During the course we were 'attacked' from clients who put in strings with special characters.
...
\end{verbatim}
We did not validate the input so it was stored in our database.

%We didn't validate user input so all the sanitized text was saved to the database text fields. 

This was a threat because it made the application usability drop when students names were not readable. 
We experienced some issues when redeployed our database because we could not drop our database. We guess that this was caused by failure to parse the special characters in the text fields which blocked the command from execution.

To resolve this we needed to delete each row individually.
...
\begin{figure}[h!]
\centering
\includegraphics[scale=0.4]{Team10_XSS}
