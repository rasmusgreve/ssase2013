...
For the integration with another team we needed to accept a self signed certificate by creating our own SSLSocketFactory. This required downloading the certificate and deploying it locally in our application so we could set it as a trusted certificate because otherwise the HTTPS client would not request the URL.
\begin{figure}
\begin{lstlisting}
